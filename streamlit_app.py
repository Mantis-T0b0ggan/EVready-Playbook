# 2. Calculate energy charges
            ...
            try:
                ...
                if not other_schedules_data:
                    ...
                    if selected_comparison_schedules:
                        comparison_results = [current_bill]
                        ...
                        html_table = create_comparison_table(comparison_results)
                        ...
                        if savings > 0 and best_option["schedule_id"] != current_bill["schedule_id"]:
                            ...
                            st.success(f"**Current Rate Optimal**: Your current rate '{current_bill['schedule_name']}' appears to be the most cost-effective option based on your usage pattern.")
...
with tab2:
    ...
    st.info("This feature will allow you to browse through rate schedules. Coming soon!")import os
import math
...
def calculate_bill(schedule_id, schedule_name, usage_kwh, demand_kw, power_factor, billing_month):
...
# Function to create a styled comparison table
def create_comparison_table(comparison_results):
    """Create an HTML table for rate comparison."""
    ...
    # Add headers for each rate
    for i, result in enumerate(comparison_results):
        if i == 0:
            html += f'<th>Option {i+1} - Current Rate</th>'
        else:
            html += f'<th>Option {i+1}</th>'
    
    html += '</tr><tr><td class="rate-name-cell">Rate Name</td>'
    
    # Add rate names
    for result in comparison_results:
        schedule_name = result["schedule_name"].split(" - ")[0] if " - " in result["schedule_name"] else result["schedule_name"]
        html += f'<td>{schedule_name}</td>'
    
    html += '</tr><tr><td class="rate-name-cell">Present</td>'
    
    # Add present costs
    for result in comparison_results:
        cell_class = " best-rate" if result["schedule_id"] == best_rate_id else ""
        html += f'<td class="{cell_class}">${result["total"]:.2f}</td>'
    
    html += '</tr><tr><td class="rate-name-cell">Future (Projected)</td>'
    
    # Add projected costs
    for result in comparison_results:
        projected_best = min(comparison_results, key=lambda x: x["projected"])["schedule_id"]
        cell_class = " best-rate" if result["schedule_id"] == projected_best else ""
        html += f'<td class="{cell_class}">${result["projected"]:.2f}</td>'
